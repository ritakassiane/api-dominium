...
function api_get_direcao (){
    $posts = array();
    $args = array('post_type' => 'direcao', 'post_per_page' => -1);
    $loop = new WP_Query($args);
    while ($loop -> have_posts()) : $loop->the_post();
        $id = get_the_ID();
        $slug = get_post_field('post_name', $id);
...
function api_get_professores_i (){
    $posts = array();
    $args = array('post_type' => 'professores_i', 'post_per_page' => -1);
    $loop = new WP_Query($args);
    while ($loop -> have_posts()) : $loop->the_post();
        $id = get_the_ID();
        $slug = get_post_field('post_name', $id);
...
function api_get_professores_ii (){
    $posts = array();
    $args = array('post_type' => 'professores_ii', 'post_per_page' => -1);
    $loop = new WP_Query($args);
    while ($loop -> have_posts()) : $loop->the_post();
        $id = get_the_ID();
        $slug = get_post_field('post_name', $id);
...
add_action('rest_api_init', 'api_register_direcao');
